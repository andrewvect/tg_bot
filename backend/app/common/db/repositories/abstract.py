...
from sqlalchemy.sql.elements import BinaryExpression, BooleanClauseList, ColumnElement
...
WhereClause = ColumnElement[bool] | BinaryExpression[bool] | BooleanClauseList
...
class Repository(Generic[AbstractModel]):
    ...
    async def get(self, identifier: int | str) -> AbstractModel | None:
        """Fetch a single entity by its identifier."""
        return await self.session.get(
            entity=self.type_model, ident=identifier, populate_existing=True
        )

    async def get_by_condition(self, condition: WhereClause) -> AbstractModel | None:
    ...
    async def update(self, model: AbstractModel) -> AbstractModel:
        """Update an entity"""
